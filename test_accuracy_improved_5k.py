#!/usr/bin/env python3
"""
Improved accuracy test script for 5K images with better LSH parameters.
This version addresses the issue of creating one massive group.
"""
...
def create_enhanced_whash_deduplicator() -> EnhancedWHashDeduplicator:
    """Create improved Enhanced WHash deduplicator with better LSH parameters."""
    return EnhancedWHashDeduplicator(
        ...
        enable_lsh=True,
        lsh_bands=12,  # Increased for better granularity
        lsh_rows_per_band=2,  # Reduced for better granularity
        similarity_threshold=0.75  # More conservative threshold
    )
...
def create_hybrid_similarity_calculator(
    ...
    return HybridSimilarityCalculator(
        ...
        thresholds={'global': 0.55, 'structural': 0.50, 'color': 0.45, 'whash': 0.50},  # More conservative
        enable_caching=True,
...
def create_accuracy_optimized_deduplicator(
    ...
    return AccuracyOptimizedDeduplicator(
        ...
        verification_threshold=0.55,  # More conservative threshold
        max_group_size=150,  # Reduced for better performance
        enable_caching=True
    )
...
def run_accuracy_improved_test(image_paths: List[str]) -> Dict[str, Any]:
    """Run accuracy-improved test with better LSH parameters."""
    logger.info("🔸 Running accuracy-improved test (better LSH parameters)...")
    start_time = time.time()
    ...
    try:
        ...
        logger.info(f"✅ Accuracy-Improved: {total_duplicates} duplicates in {processing_time:.2f}s")
...
def main():
    """Main test function."""
    logger.info("🚀 Starting improved accuracy test for 5K images...")
    ...
    # Accuracy-improved test
    results['accuracy_improved'] = run_accuracy_improved_test(image_paths)
    
    # Print comparison
    logger.info("=" * 80)
    logger.info("📊 IMPROVED DEDUPLICATION COMPARISON RESULTS")
    logger.info("=" * 80)
    ...
    logger.info(f"   - Duplicates found: {results['accuracy_improved']['duplicates']}")
    logger.info(f"   - Processing time: {results['accuracy_improved']['time']:.2f}s")
    if results['accuracy_improved']['time'] > 0:
        logger.info(f"   - Speed: {len(image_paths)/results['accuracy_improved']['time']:.1f} images/sec")
    
    # Improvement analysis
    if results['color']['duplicates'] > 0:
        improvement = ((results['accuracy_improved']['duplicates'] - results['color']['duplicates']) / results['color']['duplicates']) * 100
        logger.info(f"📈 IMPROVEMENT ANALYSIS:")
        logger.info(f"   - vs Baseline Color: {improvement:+.1f}% duplicates found")
    
    # Performance analysis
    if 'whash_stats' in results['accuracy_improved']:
        whash_stats = results['accuracy_improved']['whash_stats']
        hybrid_stats = results['accuracy_improved']['hybrid_stats']
        accuracy_stats = results['accuracy_improved']['accuracy_stats']
    ...
    logger.info("=" * 80)
    logger.info("🎉 Improved accuracy test completed successfully!")
